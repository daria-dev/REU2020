...
def train_nn(train_y,train_v,val_y,val_v,net,criterion,optimizer,args):
    ...
    OUTPUT:
        None
    '''
    assert ((len(train_y.shape) == 3) and (len(train_v.shape) == 3)),'training data should be 3D array'
    assert ((len(val_y.shape) == 3) and (len(val_v.shape) == 3)),'validation data should be 3D array'
    ...
    for epoch in range(args.num_epoch):
        for i, (train_y_batch, train_v_batch) in enumerate(train_loader):  
            ...
            def closure():
                optimizer.zero_grad()
                loss = criterion(net(train_y_batch), train_v_batch)
        ...
        if (epoch+1) % 500 == 0:
            with torch.no_grad():
                loss_train = criterion(net(train_y_tensor), train_v_tensor)
                loss_val = criterion(net(val_y_tensor), val_v_tensor)
