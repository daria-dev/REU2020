...
def train_nn(train_y, val_y, net, criterion, optimizer, args):
    ...
    train_prior_tensor = prior_glycolytic(train_y_tensor)
    val_prior_tensor = prior_glycolytic(val_y_tensor)

    # create batches for training data
    train_dataset = torch.utils.data.TensorDataset(train_y_tensor, train_prior_tensor, train_y1_tensor)
    ...
    for epoch in range(args.num_epoch):
        for i, (train_y_batch, train_prior_batch, train_y1_batch) in enumerate(train_loader):
            current_step += 1

            def closure():
                optimizer.zero_grad()
                loss = criterion(net(train_y_batch, train_prior_batch), train_y1_batch)

                ind = -1 if args.prior_mode == 'all' else -5

                l_net = torch.tensor(0.0)
                Lambda1 = 1e-6
                for w in list(net.parameters())[:ind]:
                    # print(w.shape)
                    l_net += w.norm()
                loss += l_net * Lambda1
        ...
        if (epoch + 1) % 100 == 0:
            with torch.no_grad():
                loss_train = criterion(net(train_y_tensor, train_prior_tensor), train_y1_tensor)
                loss_val = criterion(net(val_y_tensor, val_prior_tensor), val_y1_tensor)
                train_list.append(loss_train)
                val_list.append(loss_val)
                epoch_list.append(epoch + 1)

            print_epoch(epoch, args.num_epoch, loss_train.item(), loss_val.item(),
...
def prior_glycolytic(x, numpy=False):
    # S2*S5, S3*S6, S4*S5
    ...
    return torch.stack([s2s5, s3s6, s4s5], dim=-1)
