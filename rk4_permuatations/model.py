...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, nn_depth):
        ...
        self.hidden.append(nn.Linear(hidden_dim, 1))
...
class RK4(nn.Module):
    ...
    def forward(self, x0):
        x1 = self.dt * get_velocity(self.f, x0)
        x2 = self.dt * get_velocity(self.f, x0 + x1/2.)
        x3 = self.dt * get_velocity(self.f, x0 + x2/2.)
        x4 = self.dt * get_velocity(self.f, x0 + x3)
